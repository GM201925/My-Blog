...
如果每次都从L算到R，时间复杂度都是$O(N)$，如果计算频繁，就比较慢

利用线段树，我们可以做到查询某个区间的和以及更新区间都是$O(log N)$的复杂度
线段树可以支持任何区间聚合操作，比如区间里找最大/最小值，我们以区间求和为例来说明。

把整个数组区间不断二分，每个节点代表一个连续区间，得到一棵树，我们可以补上一定的空间使其成为完全二叉树，一般需要$4N$的空间，图中的例子如果6结点下面还会多出两个区间（比如多一个数，3放在数组中6号位置），那这时5结点下面还要多出两个空的空间来构成完全二叉树，$2N-1$未必够用
```
          1
       /     \
      2       3
